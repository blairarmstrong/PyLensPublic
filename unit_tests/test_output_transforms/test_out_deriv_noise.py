...
def xor_runner():
    ...
    xor_net_one.add_group(
        ...
        output_transforms=["sigmoid", "out_deriv_noise"]
    )
    ...
    # Output deriv noise = 0
    # Error: 2.81169, weight cost = 1.69281
    # Output deriv noise = 0.5
    # Error: 2.87055, weight cost = 3.07823


    decimal = 3
    clens_error = round(2.81169, decimal)
    clens_weight_cost = round(1.69281, decimal)
    ...
    print("     Error: ... ", end="")
    sim.delete_all_nets()
    if not ((np.isnan(clens_error) or clens_error > 100000000) and
            (np.isnan(pylens_error) or pylens_error > 100000000)):
