...
def xor_runner():
    ...
    decimal = 3
    clens_error = round(2.90654, decimal)
    clens_weight_cost = round(1.68469, decimal)
    ...
    print("     Error: ... ", end="")
    sim.delete_all_nets()
    if not ((np.isnan(clens_error) or clens_error == 200000000) and
            (np.isnan(pylens_error) or  pylens_error == 200000000)):
