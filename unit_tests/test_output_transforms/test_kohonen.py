...
def xor_runner():
    ...
    xor_net_one = sim.create_net(name="xor", add_bias=False)
    ...
    for group in xor_net_one.groups:
        print("group name: ", group.name)
        print(group.input_transforms)
        print(group.output_transforms)



    # load example set
    xor_net_one.load_example_set("../../examples/lens_example_input/kohonen.ex")

    # load sample weights from clens random weight seed
    xor_net_one.load_clen_weight("../../examples/example_networks/kohonen/kohonen_test1_weights.wt")

    # set learning rate and update method
    xor_net_one.set_update_method(learning_rate, update_method)
    # xor_net_one.optimizer.momentum = momentum
    ...
    clens_error = round(2.80825, 3)
    clens_weight_cost = round(1.66036, 3)
