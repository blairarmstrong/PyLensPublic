...
def boltz_big_unit_test(parallel=False):
    ...
    actual_weight_cost = digits_net_one.stats_plotter.progress_stats['weight_cost'][-1]
    actual_error = digits_net_one.stats_plotter.progress_stats['error'][-1]
    sim_one.delete_all_nets()
    del(sim_one)
    
    expected_weight_cost = 721.033
    check_difference(actual_weight_cost, expected_weight_cost)
    
    expected_error = 1.88509
