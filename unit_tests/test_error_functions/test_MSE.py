...
def xor_runner(parallel, update_method="steepest"):
    ...
    # create output layer with 1 units; used sigmoid output, dot product input, squared error
    xor_net_one.add_group(
        1,
        name="output",
        group_type="output",
    ...
    if update_method == 'steepest':
        clens_error = round(0.96059, 3)
        clens_weight_cost = round(3.65266, 3)
        clens_grad_linearity = round(0.99999, 3)
    elif update_method == 'dougs momentum':
        clens_error = round(0.00630, 3)
        clens_weight_cost = round(157.427, 3)
        clens_grad_linearity = round(0.97592, 3)
    elif update_method == 'momentum':
        clens_error = round(0.00630, 3)
        clens_weight_cost = round(157.427, 3)
        clens_grad_linearity = round(0.97592, 3)
    elif update_method == 'delta bar delta':
        clens_error = round(0.00124, 3)
        clens_weight_cost = round(261.659, 3)
        clens_grad_linearity = round(0.88389, 3)
    elif update_method == 'adam':
        pytorch_error = round(0.6674059629440308, 3)
