...
def rand_runner(parallel=False):
    batch_size = 0
    report_interval = 1
    learning_rate = 0.5
    ...
    clens_error = round(12.5516, 2)
    clens_weight_cost = round(277.671, 3)

    pylens_error = round(rand10x40.stats_plotter.progress_stats['error'][-1].item(), 2)
    pylens_weight_cost = round(rand10x40.stats_plotter.progress_stats["weight_cost"][-1].item(), 3)
