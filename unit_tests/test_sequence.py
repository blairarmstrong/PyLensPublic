...
def test_sequence():
    ...
    # create simulator
    sim = Simulator(name="sim_seq")

    # create sample network
    xor_net_one = sim.create_net(name='sequence', time_intervals=4, ticks_per_interval=5)
    ...
    # create input layer with 5  units
    ...
    # create hidden layer with 1 units; used sigmoid output, output_integration, dot product input
    xor_net_one.add_group(
        3,
        name="hidden",
        group_type="hidden",
    ...
    # load example set
    xor_net_one.load_example_set("../examples/lens_example_input/sequence.ex")
