...
def rand_runner(parallel=False, extra_maxTime=False):
    ...
    if parallel == True:
        rand10x40.train(5, batch_size, report_interval, parallel_mode=True, num_worker=3)
    else:
        rand10x40.train(5, batch_size, report_interval)

    rand10x40.store_links('test_rand10x40_weight.json', weight_only=True, format='json')
    rand10x40.reset_network()
    rand10x40.load_links('test_rand10x40_weight.json')
