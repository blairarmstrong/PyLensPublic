...
def continuous_xor_runner(parallel):
    ...
    xor_net_one = sim.create_net(name='xor', type='continuous')
    ...
    if parallel == True:
        xor_net_one.train(num_updates, batch_size, report_interval, parallel_mode=True, num_worker=2)
    else:
        xor_net_one.train(num_updates, batch_size, report_interval)

    # COMPARE ERRORS AND WEIGHT COST

    clens_error = round(2.80984, 3)
    clens_weight_cost = round(1.70604, 3)
    clens_unit_cost = round(3.45662, 3)
