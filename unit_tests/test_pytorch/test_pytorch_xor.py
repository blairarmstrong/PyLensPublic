...
def xor_runner(parallel, update_method="steepest"):
    ...
    # create simulator
    sim = Simulator(name="sim_xor", baseType='pytorch')
    ...
    pylens_error = round(float(xor_net_one.stats_plotter.progress_stats['error'][-1]), 3)
    pylens_weight_cost = round(float(xor_net_one.stats_plotter.progress_stats["weight_cost"][-1]), 3)
    pylens_gradient_linearity = round(float(xor_net_one.stats_plotter.progress_stats["grad_lin"][-1]), 3)
