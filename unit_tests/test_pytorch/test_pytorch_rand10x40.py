...
def rand_runner(parallel=False, extra_maxTime=False):
    ...
    pylens_error = round(float(rand10x40.stats_plotter.progress_stats['error'][-1]), 2)
    pylens_weight_cost = round(float(rand10x40.stats_plotter.progress_stats["weight_cost"][-1]), 3)
