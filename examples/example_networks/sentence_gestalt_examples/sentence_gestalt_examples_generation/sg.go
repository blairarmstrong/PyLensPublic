...
func main() {
    ...
    sim := &Sim{}
    sim.New()
    sim.ConfigAll() // this sets up SentGenEnv and loads sg_rules.txt

    // if generate test data
	f, err := os.Create("sg_train_data.jsonl")
	// if generate test data
    // f, err := os.Create("sg_test_data.jsonl")
    if err != nil {
        panic(err)
    }
    defer f.Close()
    ...
    // if generate test data
	ev := sim.Envs.ByMode(etime.Train).(*SentGenEnv)
	// if generate test data
    // ev := sim.Envs.ByMode(etime.Test).(*SentGenEnv)

    // how many sentences to dump (change this number if you want)
    const N = 10000

    for i := 0; i < N; i++ {
        // generate one sentence + build the query sequence using their code
        ev.NextSent()

        // copy role→filler states
        st := make(map[string]string, len(ev.Rules.States))
        for k, v := range ev.Rules.States {
            st[k] = v
        }

        // copy the [word, role, filler, status] sequence
        seq := make([][4]string, len(ev.SentInputs))
        for j, in := range ev.SentInputs {
            seq[j] = [4]string{in[0], in[1], in[2], in[3]}
        }

        // optional: sentence type (last fired non-root rule)
        sentType := ""
        for n := range ev.Rules.Fired {
            if n != "Sentences" {
                sentType = n
            }
        }
        ...
        rec := map[string]any{
            "idx":            i,
            "tokens":         append([]string(nil), ev.CurSent...),
            "surface":        strings.Join(ev.CurSent, " "),
            "states":         st,   // e.g., Agent, Action, Patient, Mod, etc.
            "query_sequence": seq,  // training steps: [word, role, filler, status]
            "sent_type":      sentType,
        }
        _ = enc.Encode(rec)
    }
